...
- [`docs/Intelligence-Engine-Operations-Guide.docx`](docs/Intelligence-Engine-Operations-Guide.docx) — the **illustrated Word edition**: every role, every activity, with rendered diagrams and real console/Claude Code screenshots (account identifier redacted). Regenerate with [`scripts/build_role_guide_docx.py`](scripts/build_role_guide_docx.py)
